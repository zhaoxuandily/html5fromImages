...
这里主要增加里一个`bin`的配置，bin里的`html5fromImages`为cli的具体命令，它的具体执行的文件html5fromImages.js。

既然`html5fromImages`的执行文件是`html5fromImages.js`，我们当然需要创建创建它
...
设置了npm镜像 http://npmjs.taobao.org/
重置为官方镜像
